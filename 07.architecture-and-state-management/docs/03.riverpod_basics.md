...
```yaml
dependencies:
  flutter_riverpod: ^x.y.z # The core library
  riverpod_annotation: ^x.y.z # Annotations for code generation

dev_dependencies:
  build_runner: ^x.y.z # Runs the code generator
  riverpod_generator: ^x.y.z # The actual generator
```
...
> [!TIP]
> You don't need to read `.g.dart` files. They are an implementation detail. Treat it like compiled output: generated, and never maintained by hand.
