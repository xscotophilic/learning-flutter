# MyStore Backend API Specification

Base URL: `http://localhost:3000` (port configurable via `PORT` env var)

## Authentication

This API is served under two independent, parallel versions that share the same controllers/services/models/database. The only differences are route registration and the authentication middleware used.

### v1 - `/api/v1/*`

v1 does **not** perform real authentication. Every request is treated as having already authenticated as a fixed demo user (`demo-user`), regardless of any headers sent. This lets students focus purely on making HTTP requests without worrying about auth.

### v2 - `/api/v2/*`

v2 uses real authentication: Google Sign-In followed by the API's own JWT.

1. Sign in with Google on the client to obtain a Google ID token.
2. Exchange it for an application token:
   ...
   ```json
   {
     "data": {
       ...
       "user": { "id": "1", "email": "...", "name": "...", "picture": "..." }
     }
   }
   ```

3. Send the application JWT on all subsequent requests:

   ```
   Authorization: Bearer <application JWT>
   ```

The Google ID token is only ever used during login; it is verified once by `/api/v2/auth/google` and never accepted directly by any other endpoint.

Requests to protected v2 endpoints without a valid Bearer token receive:
...
- **Auth required:** Yes for v2 (`Authorization` header), No for v1
- **Query params:** None
- **Body:** None
